...
\node[font=\large] at (0,0) {$dZ^{[l]}=dA^{[l]}\odot\phi'(Z^{[l]}),\qquad
      dA^{[l-1]}=(W^{[l]})^\top dZ^{[l]}$};
\node[stage] at (0,-.75) {反向传播是向量—Jacobian 乘积：先过逐元素激活 Jacobian，再过线性层转置};
\DenseBlock{obj0}{-11.50}{-1.45}{4}{5}{roleCoral}{$dA^{[l]}$}{$n_l\times m$}
\DenseBlock{obj1}{-3.83}{-1.45}{4}{5}{roleGray}{$\phi'(Z^{[l]})$}{$n_l\times m$}
\DenseBlock{obj2}{3.83}{-1.45}{4}{5}{roleViolet}{$dZ^{[l]}$}{$n_l\times m$}
\DenseBlock{obj3}{11.50}{-1.45}{3}{5}{roleTeal}{$dA^{[l-1]}$}{$n_{l-1}\times m$}
\node[op] at (-7.67,-2.25) {$\odot$};
\node[op] at (0.00,-2.25) {$=$};
\node[op] at (7.67,-2.25) {$\xrightarrow{\ (W^{[l]})^\top\times\ }$};
\MeaningBox{激活反向保持 $n_l$ 与 $m$；线性反向收缩 $n_l$，输出前一层通道 $n_{l-1}$ 与批次 $m$。}
{$dA$ 是上游协向量，激活导数是逐元素局部 Jacobian 的对角表示，$dZ$ 是中间梯度。}
{框架通常不构造完整 Jacobian，而是按计算图逆序直接计算 VJP，节省大量内存。}
