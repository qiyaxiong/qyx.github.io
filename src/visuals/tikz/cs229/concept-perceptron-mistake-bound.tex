...
\node[font=\large] at (0,0) {$y_i(u^\top x_i)\ge\gamma,\ \|x_i\|\le R
      \quad\Longrightarrow\quad M\le\left(\frac R\gamma\right)^2$};
\node[stage] at (0,-.75) {可分数据上的错误上界来自两条夹逼：朝正确方向线性进步，而参数范数至多平方根增长};
\ScalarBlock{obj0}{-11.50}{-1.45}{1}{1}{roleTeal}{$\gamma$}{最小几何间隔}
\ScalarBlock{obj1}{-5.75}{-1.45}{1}{1}{roleOrange}{$M\gamma$}{正确方向进展下界}
\ScalarBlock{obj2}{0.00}{-1.45}{1}{1}{roleViolet}{$\|w_M\|$}{参数范数}
\ScalarBlock{obj3}{5.75}{-1.45}{1}{1}{roleCoral}{$R\sqrt M$}{范数增长上界}
\ScalarBlock{obj4}{11.50}{-1.45}{1}{1}{roleGray}{$(R/\gamma)^2$}{错误次数上界}
\node[op] at (-8.63,-2.25) {$\times M$};
\node[op] at (-2.88,-2.25) {$\le u^\top w_M\le$};
\node[op] at (2.88,-2.25) {$\le$};
\node[op] at (8.63,-2.25) {$\Longrightarrow M\le$};
\MeaningBox{这些量都是标量界；$R$ 与 $\gamma$ 必须使用同一特征空间中的范数和距离定义。}
{$M$ 是离散错误计数，$\gamma$ 是可分间隔，$R$ 是样本半径；该结论不是概率保证。}
{间隔越大或样本半径越小，上界越紧；不可分数据不满足前提，需要容错或在线损失界。}
