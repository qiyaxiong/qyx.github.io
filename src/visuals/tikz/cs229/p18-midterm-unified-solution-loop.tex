...
\node[font=\large] at (0,0) {$\text{assumption}\to p(y\mid x;\theta)\to
      J(\theta)\to\nabla J(\theta)\to\hat\theta\to\text{diagnosis}$};
\node[stage] at (0,-.75) {期中核心模型共享同一解题闭环：先声明分布假设，再推目标、梯度、求解与验证};
\DenseBlock{obj0}{-11.50}{-1.45}{4}{3}{roleTeal}{$X$}{$m\times n$}
\DenseBlock{obj1}{-5.75}{-1.45}{3}{1}{roleOrange}{$\theta$}{$n\times1$}
\DenseBlock{obj2}{0.00}{-1.45}{4}{1}{roleViolet}{$h_\theta(X)$}{$m\times1$}
\DenseBlock{obj3}{5.75}{-1.45}{4}{1}{roleCoral}{$\ell_i$}{$m\times1$}
\DenseBlock{obj4}{11.50}{-1.45}{3}{1}{roleOrange}{$\nabla_\theta J$}{$n\times1$}
\node[op] at (-8.63,-2.25) {$\times$};
\node[op] at (-2.88,-2.25) {$\operatorname{predict}$};
\node[op] at (2.88,-2.25) {$\xrightarrow{\ell}$};
\node[op] at (8.63,-2.25) {$\xrightarrow{\sum_m,\ \partial/\partial\theta}$};
\MeaningBox{预测和逐样本损失保留 $m$；梯度归约样本轴并回到与参数相同的 $n$ 维。}
{模型类型改变 $h,\ell$ 的具体形式，但参数、预测、损失和梯度的角色保持稳定。}
{每次推导都检查输出域、维度、符号、凸性和数值稳定性，可覆盖线性回归、Logistic、GLM 与 SVM。}
