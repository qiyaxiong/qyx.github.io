...
\node[font=\large] at (0,0) {$A^{[0]}\xrightarrow{W^{[1]}}A^{[1]}
      \xrightarrow{W^{[2]}}A^{[2]},\qquad
      W^{[l]}:(n_{l-1}\times m)\to(n_l\times m)$};
\node[stage] at (0,-.75) {每一层只改变通道轴 $n_l$，批次轴 $m$ 从输入到输出始终保持};
\DenseBlock{obj0}{-11.50}{-1.45}{3}{5}{roleTeal}{$A^{[0]}$}{$n_0\times m$}
\DenseBlock{obj1}{0.00}{-1.45}{4}{5}{roleViolet}{$A^{[1]}$}{$n_1\times m$}
\DenseBlock{obj2}{11.50}{-1.45}{2}{5}{roleCoral}{$A^{[2]}$}{$n_2\times m$}
\node[op] at (-5.75,-2.25) {$\xrightarrow{\ W^{[1]},\,b^{[1]},\,\phi^{[1]}\ }$};
\node[op] at (5.75,-2.25) {$\xrightarrow{\ W^{[2]},\,b^{[2]},\,\phi^{[2]}\ }$};
\MeaningBox{n0、n1、n2 是逐层通道数，m 是共享批次轴；相邻层的共同通道边必须同长。}
{激活矩阵按列存样本，权重矩阵只在线性变换时收缩前一层通道。}
{深度来自多次“线性变换+非线性”；若省略激活，多层线性矩阵可合并为单层。}
