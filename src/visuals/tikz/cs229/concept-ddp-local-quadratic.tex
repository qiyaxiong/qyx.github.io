...
\node[font=\large] at (0,0) {$Q_x=\ell_x+f_x^\top V_x',\quad
      Q_u=\ell_u+f_u^\top V_x',\quad
      \delta u^\star=k+K\delta x,\quad k=-Q_{uu}^{-1}Q_u$};
\node[stage] at (0,-.75) {DDP 沿名义轨迹局部二次化代价与动力学，反向求控制修正，再正向生成新轨迹};
\DenseBlock{obj0}{-11.50}{-1.45}{5}{3}{roleTeal}{$(\bar x_t,\bar u_t)$}{$T\times(n+p)$ 名义轨迹}
\DenseBlock{obj1}{-5.75}{-1.45}{3}{3}{roleViolet}{$Q_{xx},Q_{ux},Q_{uu}$}{局部二次块}
\DenseBlock{obj2}{0.00}{-1.45}{3}{1}{roleOrange}{$k_t$}{$p\times1$ 前馈}
\DenseBlock{obj3}{5.75}{-1.45}{3}{4}{roleCoral}{$K_t$}{$p\times n$ 反馈}
\DenseBlock{obj4}{11.50}{-1.45}{5}{3}{roleTeal}{$(x_t^+,u_t^+)$}{$T\times(n+p)$ 新轨迹}
\node[op] at (-8.63,-2.25) {$\operatorname{local\ model}$};
\node[op] at (-2.88,-2.25) {$\operatorname{backward}$};
\node[op] at (2.88,-2.25) {$+$};
\node[op] at (8.63,-2.25) {$\operatorname{rollout}$};
\MeaningBox{$T$ 是时间轴，$n$ 是状态轴，$p$ 是控制轴；$K_t$ 把状态偏差映射为控制偏差。}
{$k_t$ 是开环前馈修正，$K_t$ 是局部反馈增益，二者来自控制 Hessian 的线性求解。}
{反向通道计算局部策略，正向通道用真实非线性动力学 rollout；线搜索与正则化保证改进稳定。}
