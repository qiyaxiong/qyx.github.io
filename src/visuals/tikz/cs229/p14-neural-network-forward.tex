...
\node[font=\large] at (0,0) {$Z^{[l]}=W^{[l]}A^{[l-1]}+b^{[l]},\qquad
      A^{[l]}=\phi^{[l]}(Z^{[l]})$};
\node[stage] at (0,-.75) {批量前向传播：线性层在通道轴上收缩，偏置沿样本轴广播，激活保持形状};
\DenseBlock{obj0}{-11.50}{-1.45}{3}{5}{roleTeal}{$A^{[l-1]}$}{$n_{l-1}\times m$}
\DenseBlock{obj1}{-5.75}{-1.45}{4}{3}{roleOrange}{$W^{[l]}$}{$n_l\times n_{l-1}$}
\DenseBlock{obj2}{0.00}{-1.45}{4}{5}{roleViolet}{$Z^{[l]}$}{$n_l\times m$}
\DenseBlock{obj3}{5.75}{-1.45}{4}{1}{roleCoral}{$b^{[l]}$}{$n_l\times1$}
\DenseBlock{obj4}{11.50}{-1.45}{4}{5}{roleViolet}{$A^{[l]}$}{$n_l\times m$}
\node[op] at (-8.63,-2.25) {$\leftarrow\times$};
\node[op] at (-2.88,-2.25) {$=$};
\node[op] at (2.88,-2.25) {$+\operatorname{broadcast}_m$};
\node[op] at (8.63,-2.25) {$\xrightarrow{\phi}$};
\MeaningBox{行轴是神经元/通道，列轴是样本；$n_{l-1}$ 被收缩，$n_l,m$ 保留。}
{$Z$ 是激活前值，$A$ 是激活后表示，$W,b$ 是该层共享参数。}
{逐元素激活不改变形状；跨样本向量化只增加批次列，不复制参数。}
\Identity{神经网络批量前向传播}
