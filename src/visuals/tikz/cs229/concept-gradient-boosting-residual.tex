...
\node[font=\large] at (0,0) {$r_i^{(t)}=-\left.\frac{\partial\ell(y_i,F(x_i))}
      {\partial F(x_i)}\right|_{F=F_{t-1}},\quad
      F_t=F_{t-1}+\nu\rho_t h_t$};
\node[stage] at (0,-.75) {Gradient Boosting 在函数输出空间计算负梯度，再训练弱学习器拟合这一修正方向};
\DenseBlock{obj0}{-11.50}{-1.45}{5}{1}{roleTeal}{$F_{t-1}(X)$}{$m\times1$ 分数}
\DenseBlock{obj1}{-5.75}{-1.45}{5}{1}{roleCoral}{$r^{(t)}$}{$m\times1$ 伪残差}
\DenseBlock{obj2}{0.00}{-1.45}{5}{1}{roleViolet}{$h_t(X)$}{$m\times1$}
\ScalarBlock{obj3}{5.75}{-1.45}{1}{1}{roleOrange}{$\rho_t$}{标量步长}
\DenseBlock{obj4}{11.50}{-1.45}{5}{1}{roleTeal}{$F_t(X)$}{$m\times1$ 分数}
\node[op] at (-8.63,-2.25) {$-\nabla_F\ell_y$};
\node[op] at (-2.88,-2.25) {$\operatorname{fit}$};
\node[op] at (2.88,-2.25) {$\operatorname{line\ search}$};
\node[op] at (8.63,-2.25) {$+\nu\rho_t h_t$};
\MeaningBox{所有预测和伪残差共享样本轴 $m$；线搜索沿 $m$ 聚合损失后得到标量步长。}
{伪残差是连续负梯度，不一定等于 $y-\hat y$；$h_t$ 是新弱学习器，$\nu$ 是 shrinkage。}
{每轮只拟合当前函数空间下降方向；小学习率配合更多轮数通常更稳，但训练更慢。}
