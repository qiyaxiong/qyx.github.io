...
\node[font=\large] at (0,0) {$\bar f(x)=\frac1B\sum_{b=1}^Bf_b(x),\qquad
      \operatorname{Var}(\bar f)=\rho\sigma^2+\frac{1-\rho}{B}\sigma^2$};
\node[stage] at (0,-.75) {Bootstrap 产生多份训练集，基学习器给出相关预测，平均沿模型轴降低非相关方差};
\DenseBlock{obj0}{-11.50}{-1.45}{5}{3}{roleTeal}{$\mathcal D$}{$m\times n$}
\DenseBlock{obj1}{-5.75}{-1.45}{4}{5}{roleOrange}{Bootstrap IDs}{$B\times m$ 整数}
\DenseBlock{obj2}{0.00}{-1.45}{4}{1}{roleViolet}{$f_b(x)$}{$B\times1$}
\ScalarBlock{obj3}{5.75}{-1.45}{1}{1}{roleCoral}{$\bar f(x)$}{标量}
\ScalarBlock{obj4}{11.50}{-1.45}{1}{1}{roleGray}{$\operatorname{Var}$}{标量}
\node[op] at (-8.63,-2.25) {$\operatorname{resample}$};
\node[op] at (-2.88,-2.25) {$\operatorname{fit}_B$};
\node[op] at (2.88,-2.25) {$\operatorname{mean}_B$};
\node[op] at (8.63,-2.25) {$\longrightarrow$};
\MeaningBox{$B$ 是模型/重采样轴，$m$ 是每份样本数；平均只消去模型轴。}
{Bootstrap IDs 是离散带放回索引，不是权重；$f_b(x)$ 是各模型对同一查询的预测。}
{增大 $B$ 只能压低 $(1-\rho)\sigma^2/B$；随机森林通过特征随机化进一步降低相关性 $\rho$。}
