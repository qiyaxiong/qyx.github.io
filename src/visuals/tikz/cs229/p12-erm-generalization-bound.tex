...
\node[font=\large] at (0,0) {$\Pr\!\left(\sup_{h\in\mathcal H}
      |\hat\varepsilon(h)-\varepsilon(h)|>\gamma\right)
      \le 2|\mathcal H|e^{-2m\gamma^2}$};
\node[stage] at (0,-.75) {经验误差在样本轴上平均；统一收敛界再同时控制整个假设类中的最大偏差};
\DenseBlock{obj0}{-11.50}{-1.45}{4}{5}{roleTeal}{$\mathbf1[h_l(x_i)\ne y_i]$}{$L\times m$}
\DenseBlock{obj1}{-5.75}{-1.45}{4}{1}{roleOrange}{$\hat\varepsilon(h_l)$}{$L\times1$}
\DenseBlock{obj2}{0.00}{-1.45}{4}{1}{roleViolet}{$\varepsilon(h_l)$}{$L\times1$}
\DenseBlock{obj3}{5.75}{-1.45}{4}{1}{roleCoral}{$|\hat\varepsilon-\varepsilon|$}{$L\times1$}
\ScalarBlock{obj4}{11.50}{-1.45}{1}{1}{roleGray}{$\sup_h$}{标量}
\node[op] at (-8.63,-2.25) {$\operatorname{mean}_m$};
\node[op] at (-2.88,-2.25) {$-$};
\node[op] at (2.88,-2.25) {$|\cdot|$};
\node[op] at (8.63,-2.25) {$\max_L$};
\MeaningBox{$m$ 是样本轴，$L=|\mathcal H|$ 是有限假设轴；Union Bound 为每个假设付出复杂度代价。}
{$\hat\varepsilon$ 是经验风险，$\varepsilon$ 是未知真风险，$\gamma$ 是允许偏差。}
{样本数增大使集中项指数下降；假设类越大，控制全部假设所需样本越多。}
