...
\node[font=\large] at (0,0) {$Z=XW+\mathbf1_m b^\top,\qquad
      (m\times n)(n\times p)+(m\times1)(1\times p)\to m\times p$};
\node[stage] at (0,-.75) {向量化把样本批次一次送入矩阵乘法；广播只复制缺失轴，不改变单个偏置的语义};
\DenseBlock{obj0}{-11.50}{-1.45}{5}{3}{roleTeal}{$X$}{$m\times n$}
\DenseBlock{obj1}{-5.75}{-1.45}{3}{4}{roleOrange}{$W$}{$n\times p$}
\DenseBlock{obj2}{0.00}{-1.45}{5}{4}{roleViolet}{$XW$}{$m\times p$}
\DenseBlock{obj3}{5.75}{-1.45}{1}{4}{roleCoral}{$b^\top$}{$1\times p$}
\DenseBlock{obj4}{11.50}{-1.45}{5}{4}{roleViolet}{$Z$}{$m\times p$}
\node[op] at (-8.63,-2.25) {$\times$};
\node[op] at (-2.88,-2.25) {$=$};
\node[op] at (2.88,-2.25) {$+\operatorname{broadcast}_m$};
\node[op] at (8.63,-2.25) {$=$};
\MeaningBox{$n$ 是矩阵乘法收缩轴；$m$ 是批次轴；偏置只拥有输出轴 $p$，沿 $m$ 被广播。}
{$W$ 是线性变换，$b$ 是每个输出通道共享的偏置，$Z$ 与批量输出同形。}
{广播要求从尾轴对齐且维度相等或为 $1$；显式写出目标形状能避免静默算错。}
