...
\node[font=\large] at (0,0) {$p(\theta\mid D)=
      \frac{p(D\mid\theta)p(\theta)}{\int p(D\mid\vartheta)p(\vartheta)\,d\vartheta}$};
\node[stage] at (0,-.75) {Bayes 更新：先验与似然逐点相乘，再由证据做全局归一化得到后验};
\DenseBlock{obj0}{-11.50}{-1.45}{5}{1}{roleTeal}{$p(\theta)$}{参数网格 $q\times1$}
\DenseBlock{obj1}{-5.75}{-1.45}{5}{1}{roleOrange}{$p(D\mid\theta)$}{$q\times1$}
\DenseBlock{obj2}{0.00}{-1.45}{5}{1}{roleViolet}{未归一化后验}{$q\times1$}
\ScalarBlock{obj3}{5.75}{-1.45}{1}{1}{roleGray}{$p(D)$}{标量证据}
\DenseBlock{obj4}{11.50}{-1.45}{5}{1}{roleCoral}{$p(\theta\mid D)$}{$q\times1$}
\node[op] at (-8.63,-2.25) {$\odot$};
\node[op] at (-2.88,-2.25) {$=$};
\node[op] at (2.88,-2.25) {$\xrightarrow{\sum/\int}$};
\node[op] at (8.63,-2.25) {$\xrightarrow{\div p(D)}$};
\MeaningBox{$q$ 表示示意参数网格；归一化沿全部候选参数归约，输出重新成为概率分布。}
{先验表达观测前信念，似然评价参数对数据的解释，证据是归一化常数，后验融合二者。}
{乘法在同一参数位置逐点发生；证据保证后验非负且总质量为 $1$。}
