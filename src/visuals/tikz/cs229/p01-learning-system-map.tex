...
\node[font=\large] at (0,0) {$\mathcal D\xrightarrow{\ \mathcal A\ }\theta
      \xrightarrow{\ h_\theta\ }\hat y,\qquad
      \hat R(\theta)=\frac1m\sum_{i=1}^m\ell(h_\theta(x^{(i)}),y^{(i)})$};
\node[stage] at (0,-.75) {机器学习闭环：数据经过学习算法形成参数，再由模型产生预测并接受风险评估};
\DenseBlock{obj0}{-11.50}{-1.45}{5}{3}{roleTeal}{$\mathcal D$}{$m\times(n+1)$}
\DenseBlock{obj1}{-5.75}{-1.45}{3}{1}{roleOrange}{$\theta=\mathcal A(\mathcal D)$}{$n\times1$}
\DenseBlock{obj2}{0.00}{-1.45}{5}{1}{roleViolet}{$\hat y=h_\theta(X)$}{$m\times1$}
\DenseBlock{obj3}{5.75}{-1.45}{5}{1}{roleCoral}{$\ell(\hat y,y)$}{$m\times1$}
\ScalarBlock{obj4}{11.50}{-1.45}{1}{1}{roleGray}{$\hat R(\theta)$}{标量}
\node[op] at (-8.63,-2.25) {$\xrightarrow{\mathcal A}$};
\node[op] at (-2.88,-2.25) {$\xrightarrow{h_\theta}$};
\node[op] at (2.88,-2.25) {$\xrightarrow{\ell}$};
\node[op] at (8.63,-2.25) {$\xrightarrow{\frac1m\sum}$};
\MeaningBox{$m$ 是样本轴，$n$ 是特征/参数轴；逐样本损失保留 $m$，平均风险把它归约为标量。}
{$\mathcal D$ 是带标签数据，$\mathcal A$ 是学习算法，$\theta$ 是模型参数，$\hat R$ 是经验风险。}
{训练不是直接记住输出，而是用数据选择参数；评估再把逐样本误差聚合成可优化的目标。}
