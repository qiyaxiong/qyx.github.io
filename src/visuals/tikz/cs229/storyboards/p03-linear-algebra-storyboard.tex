...
\Panel{p1}{0}{0}{1. 向量表示一个对象}{向量的每个坐标对应一个特征轴。\Formula{$x\in\mathbb R^n,\qquad \lVert x\rVert_2=\sqrt{x^\top x}$}内积同时给出长度与夹角。}
\Panel{p2}{9.55}{0}{2. 矩阵表示线性变换}{\Formula{$A:\mathbb R^n\to\mathbb R^m,\qquad y=Ax$}矩阵乘法是“先组合输入方向，再得到输出坐标”。}
\Panel{p3}{19.1}{0}{3. 秩描述有效方向}{\Formula{$\operatorname{rank}(A)\le\min(m,n)$}秩不足意味着有方向被压扁；求逆或回归时会出现不可辨识。}
\Panel{p4}{0}{-6.45}{4. 特征向量是不转向的方向}{\Formula{$Av=\lambda v$}变换只改变它的尺度。对称矩阵拥有正交特征向量，可作为新的坐标轴。}
\Panel{p5}{9.55}{-6.45}{5. SVD 拆解任意矩阵}{\Formula{$A=U\Sigma V^\top$}先沿 $V$ 旋转，按奇异值缩放，再沿 $U$ 旋转。}
\Panel{p6}{19.1}{-6.45}{6. 最小二乘使用投影}{\Insight{$Ax\approx b$ 的最优残差与 $A$ 的列空间正交：$A^\top(b-Ax)=0$。}}
\Summary{向量与内积 $\rightarrow$ 线性变换 $\rightarrow$ 秩 $\rightarrow$ 特征分解 $\rightarrow$ SVD $\rightarrow$ 投影求解。}
\Identity
\end{tikzpicture}
\end{document}
