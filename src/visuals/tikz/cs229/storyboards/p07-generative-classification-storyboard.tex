...
\Panel{p1}{0}{0}{1. 先描述数据怎样生成}{生成式模型分别学习先验与类条件分布：\Formula{$p(y),\qquad p(x\mid y)$}最后再用 Bayes 公式反推类别。}
\Panel{p2}{9.55}{0}{2. GDA 的假设}{两类数据各自服从高斯，并共享协方差：\Formula{$x\mid y=k\sim\mathcal N(\mu_k,\Sigma)$}}
\Panel{p3}{19.1}{0}{3. 参数来自计数和均值}{\Formula{$\phi=\tfrac1m\sum_i\mathbf1[y_i=1]$\\$\mu_k=\dfrac{\sum_i\mathbf1[y_i=k]x_i}{\sum_i\mathbf1[y_i=k]}$}协方差汇总所有类内偏差。}
\Panel{p4}{0}{-6.45}{4. 为什么边界是直线}{比较两个类别的对数后验，二次项因共享 $\Sigma$ 抵消：\Insight{$\log\dfrac{p(y=1\mid x)}{p(y=0\mid x)}=w^\top x+b$。}}
\Panel{p5}{9.55}{-6.45}{5. Naive Bayes 换一种分解}{高维离散特征使用条件独立假设：\Formula{$p(x\mid y)=\prod_j p(x_j\mid y)$}在对数空间里，乘积变成求和。}
\Panel{p6}{19.1}{-6.45}{6. 零频率必须平滑}{\Formula{$\hat\phi_{j\mid y}=\dfrac{N_{j,y}+\alpha}{N_y+\alpha K}$}拉普拉斯平滑避免一个未见词让整类概率归零。}
\Summary{建模 $p(y)$ 与 $p(x\mid y)$ $\rightarrow$ 估计参数 $\rightarrow$ 比较对数后验 $\rightarrow$ 得到分类边界。}
\Identity
\end{tikzpicture}
\end{document}
