...
\node[font=\large] at (0,0) {$p(y;\eta)=b(y)e^{\eta^\top T(y)-a(\eta)},\qquad
      \eta=\theta^\top x,\quad \mathbb E[y\mid x]=g^{-1}(\eta)$};
\node[stage] at (0,-.75) {GLM 把输入的线性预测量连接到指数族自然参数，再由分布给出条件均值};
\DenseBlock{obj0}{-11.50}{-1.45}{4}{3}{roleTeal}{$X$}{$m\times n$}
\DenseBlock{obj1}{-5.75}{-1.45}{3}{1}{roleOrange}{$\theta$}{$n\times1$}
\DenseBlock{obj2}{0.00}{-1.45}{4}{1}{roleViolet}{$\eta=X\theta$}{$m\times1$}
\DenseBlock{obj3}{5.75}{-1.45}{4}{1}{roleCoral}{$\mu=g^{-1}(\eta)$}{$m\times1$}
\DenseBlock{obj4}{11.50}{-1.45}{4}{1}{roleGray}{$p(y\mid\eta)$}{$m$ 个分布}
\node[op] at (-8.63,-2.25) {$\times$};
\node[op] at (-2.88,-2.25) {$\xrightarrow{g^{-1}}$};
\node[op] at (2.88,-2.25) {$\longrightarrow$};
\node[op] at (8.63,-2.25) {$\xrightarrow{\text{likelihood}}$};
\MeaningBox{线性层收缩特征轴 $n$，样本轴 $m$ 从自然参数一直保留到条件分布。}
{$\eta$ 是自然参数，$T(y)$ 是充分统计量，$a(\eta)$ 是对数配分函数，$\mu$ 是条件均值。}
{选择指数族分布与响应函数后，GLM 的学习仍归结为估计共享参数 $\theta$。}
\Identity{指数族到广义线性模型}
