...
\node[font=\large] at (0,0) {$p=\sigma(X\theta),\quad W=\operatorname{diag}(p_i(1-p_i)),
      \quad \theta^+=\theta-(X^\top WX)^{-1}X^\top(p-y)$};
\node[stage] at (0,-.75) {IRLS 把 Logistic 回归的局部曲率编码为逐样本对角权重，再解一个加权最小二乘步};
\DenseBlock{obj0}{-11.50}{-1.45}{5}{3}{roleTeal}{$X$}{$m\times n$}
\DenseBlock{obj1}{-5.75}{-1.45}{5}{1}{roleViolet}{$p=\sigma(X\theta)$}{$m\times1$}
\DiagBlock{obj2}{0.00}{-1.45}{5}{5}{roleOrange}{$W$}{$m\times m$}
\DenseBlock{obj3}{5.75}{-1.45}{3}{3}{roleCoral}{$X^\top WX$}{$n\times n$}
\DenseBlock{obj4}{11.50}{-1.45}{3}{1}{roleOrange}{$\Delta\theta$}{$n\times1$}
\node[op] at (-8.63,-2.25) {$\xrightarrow{\theta,\sigma}$};
\node[op] at (-2.88,-2.25) {$\xrightarrow{p(1-p)}$};
\node[op] at (2.88,-2.25) {$\xrightarrow{X^\top(\cdot)X}$};
\node[op] at (8.63,-2.25) {$\xrightarrow{\operatorname{solve}}$};
\MeaningBox{样本概率和曲率权重沿 $m$ 对齐；两次与 X 收缩后得到参数空间中的 $n\times n$ Hessian。}
{$p_i$ 是概率，$W_{ii}$ 是局部曲率而不是样本标签，$\Delta\theta$ 是 Newton 修正量。}
{边界附近 $p_i\approx0.5$ 的样本权重大；极有把握的样本曲率小，对 Newton 步影响较弱。}
