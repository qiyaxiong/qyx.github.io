...
\node[font=\large] at (0,0) {$\log p(x)=\mathcal L(q,\theta)
      +\mathrm{KL}\!\left(q(z)\,\|\,p(z\mid x;\theta)\right)$};
\node[stage] at (0,-.75) {ELBO 与证据之间的缺口恰好是非负 KL；E 步令辅助分布贴近真实后验并闭合缺口};
\DenseBlock{obj0}{-11.50}{-1.45}{4}{3}{roleTeal}{$q(z)$}{$m\times K$ 辅助分布}
\DenseBlock{obj1}{-5.75}{-1.45}{4}{3}{roleViolet}{$p(z\mid x;\theta)$}{$m\times K$ 后验}
\ScalarBlock{obj2}{0.00}{-1.45}{1}{1}{roleCoral}{$\mathrm{KL}(q\|p)$}{非负标量}
\ScalarBlock{obj3}{5.75}{-1.45}{1}{1}{roleOrange}{$\mathcal L(q,\theta)$}{标量下界}
\ScalarBlock{obj4}{11.50}{-1.45}{1}{1}{roleGray}{$\log p(x)$}{标量证据}
\node[op] at (-8.63,-2.25) {$\operatorname{compare}$};
\node[op] at (-2.88,-2.25) {$\sum q\log(q/p)$};
\node[op] at (2.88,-2.25) {$+$};
\node[op] at (8.63,-2.25) {$=$};
\MeaningBox{$m$ 是观测轴，$K$ 是隐状态轴；KL 对两轴求和后成为标量缺口。}
{$q$ 是可选辅助分布，后验由当前参数决定，ELBO 是可优化下界而非另一个似然。}
{E 步取 $q=p(z\mid x;\theta)$ 令 KL 为零；M 步固定 q 提高 ELBO，因此观测似然不下降。}
