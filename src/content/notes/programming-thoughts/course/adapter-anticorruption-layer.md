---
title: P34–P35：适配器与反腐层：把第三方协议挡在业务边界外
description: 以“接入字段、单位和错误语义都不同的旧支付网关”为贯穿案例，按视频顺序从朴素实现、需求失败到重构命名，渐进理解 Adapter、对象适配器。
publishDate: 2026-08-23
updatedDate: 2026-08-27
language: zh
---

这不是把 P34–P35 的定义拼成一篇词典，而是让一个系统连续经历几轮需求。我们从 **接入字段、单位和错误语义都不同的旧支付网关** 的最直接实现出发；每当代码真的承受不住下一次变化，才引入一个新边界。

最终要回答的是：**适配器除了改方法名，还必须翻译哪些业务语义？** 学完的验收标准也不是记住名词，而是能在系统边界完成协议、数据、单位与错误映射。

## 先看完整推导路线

| 阶段 | 原视频 | 要解决的问题 | 具体设计动作 | 保护的结果 |
| --- | --- | --- | --- | --- |
| 1 | P34 | 第三方接口与业务接口不一致时，转换代码应该放在哪里？ | 业务层被迫理解旧网关的字段名、单位和错误码 | 在边界实现 Target 接口并委托给 Adaptee | 核心业务继续使用自己的语言 |
| 2 | P35 | 除了改方法名，适配器还应处理哪些语义差异？ | 外部天气 API 的 null、华氏度和异常结构渗入页面 | 适配器统一完成 DTO、单位和错误语义转换 | 系统内部模型保持稳定可预测 |

这张表不是术语目录，而是一份修改记录。每一行都要能回答：变化从哪里来、边界放在哪里、什么结果不应该被碰。阅读正文时建议停在每个“新需求”之前，先自己写修改清单，再比较后面的重构。

## 本章总结：把每个 P 压缩成一张决策卡

### P34 · 适配器（一）：转换接口，不改变业务意图

**问题**：业务层被迫理解旧网关的字段名、单位和错误码。

**动作**：在边界实现 Target 接口并委托给 Adaptee。

**验证**：核心业务继续使用自己的语言。

**代价**：若能直接修改并统一被适配对象，适配层可能多余。

### P35 · 适配器（二）：把外部数据模型挡在系统边界

**问题**：外部天气 API 的 null、华氏度和异常结构渗入页面。

**动作**：适配器统一完成 DTO、单位和错误语义转换。

**验证**：系统内部模型保持稳定可预测。

**代价**：转换失败要显式返回，不能静默伪造业务值。

这些卡片故意把“代价”放在“验证”后面。先证明边界解决了真实问题，再讨论多了一层接口、注册表、包装器或组合根是否值得。没有代价说明的设计总结，通常只是模式宣传。

![适配器与反腐层：把第三方协议挡在业务边界外：同一案例随需求逐步演进](/images/notes/programming-thoughts/diagrams/chapter-adapter-anticorruption-layer.svg)

## 起点：先写最直接、能工作的版本

这一版故意把决策和实现放在一起。它不是“错误代码”，而是需求尚少时合理的最低成本方案。请先运行它，并标出每个业务决定；接下来的 P34–P35 会连续修改同一案例。

```python
result = legacy.pay(ref=order.id, amount_yuan=order.cents / 100)
if result["code"] == "0000": return result["trade_no"]
if result["code"] == "1003": raise Exception("余额不足")
```

## 第 1 轮需求：先解决问题，最后再叫它 Adapter（P34）

我们继续修改同一个“接入字段、单位和错误语义都不同的旧支付网关”，不另起一个玩具项目。本轮的新压力是：**旧网关使用元、供应商字段和数字错误码，业务只想按分收费**。先不要背模式名，先列出要改的判断、对象、外部依赖和测试；这份清单就是“修改半径”。

这一轮真正要回答的是：第三方接口与业务接口不一致时，转换代码应该放在哪里？ 这是本章的第一轮，先保留最直接的实现，记录它面对下一次需求时会触碰哪些地方。

### 先记录修改半径

- 当前泄漏：业务层被迫理解旧网关的字段名、单位和错误码。
- 设计动作：在边界实现 Target 接口并委托给 Adaptee。
- 稳定结果：核心业务继续使用自己的语言。
- 仍需承担的代价：若能直接修改并统一被适配对象，适配层可能多余。

如果四项里只有“新增一个类”而没有说明哪种变化被隔离，先不要合并这次重构。设计的证据不是角色数量，而是下一次同类需求能否只新增实现、只替换适配器或只调整组合根。

### 这一轮只做一个设计动作

本轮只落实 **适配器（一）：转换接口，不改变业务意图**，围绕“Adapter、Target、Adaptee、边界转换”移动一个边界，不同时引入后续模式。把稳定流程写成调用方真正需要的业务语言；把供应商字段、创建细节或横切动作留在边界另一侧。

完成动作后再给它命名：这一轮对应 **Adapter、Target、Adaptee、边界转换**。名称只是压缩沟通，不是推导起点。如果无法从“需求如何变化”推回这个结构，就说明我们只记住了答案。
...
先写一个会失败的测试，再写实现：新增一种“旧支付网关接入”变化时，哪些稳定代码应该保持不动？如果实现抛出异常、返回空值或收到重复调用，调用方依赖的行为是否仍然成立？这一步能防止只验证 happy path。

本轮的验收标准是：第三方接口与业务接口不一致时，转换代码应该放在哪里？ 完成后，核心业务继续使用自己的语言。如果测试仍然必须启动所有外部服务，或仍要修改同一个大分支，说明这一步只换了名字，没有形成可验证的边界。

### 与前后两轮的连接

下一轮 P35 会继续检验这条边界：如果新增需求仍要修改本轮稳定代码，说明边界还没有放在真正的变化轴上。

不要把本轮结构当成永久答案。只要真实需求证明它没有减少修改范围，就应该回退一层，删除不必要的间接调用，再重新找变化轴。

## 第 2 轮需求：先解决问题，最后再叫它 对象适配器（P35）

我们继续修改同一个“接入字段、单位和错误语义都不同的旧支付网关”，不另起一个玩具项目。本轮的新压力是：**第二个调用方出现后，金额、回执与失败语义都必须在适配边界统一**。先不要背模式名，先列出要改的判断、对象、外部依赖和测试；这份清单就是“修改半径”。

这一轮真正要回答的是：除了改方法名，适配器还应处理哪些语义差异？ 上一轮 P34 已经处理“Adapter”；这一轮不要把它推倒重来，而是只处理新的变化轴。

### 先记录修改半径

- 当前泄漏：外部天气 API 的 null、华氏度和异常结构渗入页面。
- 设计动作：适配器统一完成 DTO、单位和错误语义转换。
- 稳定结果：系统内部模型保持稳定可预测。
- 仍需承担的代价：转换失败要显式返回，不能静默伪造业务值。

如果四项里只有“新增一个类”而没有说明哪种变化被隔离，先不要合并这次重构。设计的证据不是角色数量，而是下一次同类需求能否只新增实现、只替换适配器或只调整组合根。

### 这一轮只做一个设计动作

本轮只落实 **适配器（二）：把外部数据模型挡在系统边界**，围绕“对象适配器、错误映射、DTO、反腐层”移动一个边界，不同时引入后续模式。把稳定流程写成调用方真正需要的业务语言；把供应商字段、创建细节或横切动作留在边界另一侧。

完成动作后再给它命名：这一轮对应 **对象适配器、错误映射、DTO、反腐层**。名称只是压缩沟通，不是推导起点。如果无法从“需求如何变化”推回这个结构，就说明我们只记住了答案。
...
先写一个会失败的测试，再写实现：新增一种“天气服务客户端”变化时，哪些稳定代码应该保持不动？如果实现抛出异常、返回空值或收到重复调用，调用方依赖的行为是否仍然成立？这一步能防止只验证 happy path。

本轮的验收标准是：除了改方法名，适配器还应处理哪些语义差异？ 完成后，系统内部模型保持稳定可预测。如果测试仍然必须启动所有外部服务，或仍要修改同一个大分支，说明这一步只换了名字，没有形成可验证的边界。
...
![适配器与反腐层：把第三方协议挡在业务边界外：案例 UML 结构与对象关系](/images/notes/programming-thoughts/diagrams/chapter-adapter-anticorruption-layer-uml.svg)
...
![适配器与反腐层：把第三方协议挡在业务边界外：同一份代码重构前后结构对照](/images/notes/programming-thoughts/diagrams/chapter-adapter-anticorruption-layer-comparison.svg)
...
为“接入字段、单位和错误语义都不同的旧支付网关”增加一个与现有实现明显不同的需求。第一轮禁止增加接口，只修改朴素版本并记录触碰点；第二轮使用本文对应步骤重构；第三轮再加入一个变化，验证旧流程是否保持不动。

提交物包括：重构前后的修改清单、一张依赖方向图、至少两个业务测试、一个共享契约测试，以及一段不超过 150 字的代价说明。最后回看 P34–P35，确认每个结构都能追溯到一个真实失败，而不是追溯到一个想背下来的名词。

## 小结

本章真正形成的是一条推理链：直接实现 → 新需求暴露耦合 → 找到变化轴 → 放置最小边界 → 用测试证明 → 写下代价。做到这一步，Adapter、对象适配器 才不再是彼此分散的知识点，而是同一段代码演进过程中自然出现的工具。
