---
title: P43–P44：组合模式与递归对象树：让叶子和容器接受同一种操作
description: 以“同时包含用户、角色组和组织节点的权限树”为贯穿案例，按视频顺序从朴素实现、需求失败到重构命名，渐进理解 Composite、递归遍历。
publishDate: 2026-08-23
updatedDate: 2026-08-27
language: zh
---

这不是把 P43–P44 的定义拼成一篇词典，而是让一个系统连续经历几轮需求。我们从 **同时包含用户、角色组和组织节点的权限树** 的最直接实现出发；每当代码真的承受不住下一次变化，才引入一个新边界。

最终要回答的是：**怎样统一处理单个节点和节点集合，又不牺牲类型安全？** 学完的验收标准也不是记住名词，而是能用递归协议聚合树形结果并选择透明式或安全式接口。

## 先看完整推导路线

| 阶段 | 原视频 | 要解决的问题 | 具体设计动作 | 保护的结果 |
| --- | --- | --- | --- | --- |
| 1 | P43 | 调用方如何不区分单个对象和对象集合就完成递归操作？ | 遍历文件时到处判断当前节点是文件还是目录 | 让 Leaf 与 Composite 实现统一 Node 接口 | 调用方可以用递归统一计算大小或执行操作 |
| 2 | P44 | 父节点如何聚合子节点结果，同时阻止叶子节点添加孩子？ | 权限树的父节点和叶子节点使用完全不同的处理流程 | 在组合节点递归聚合孩子结果，叶子提供基本结果 | 任意子树都能作为整体处理 |

这张表不是术语目录，而是一份修改记录。每一行都要能回答：变化从哪里来、边界放在哪里、什么结果不应该被碰。阅读正文时建议停在每个“新需求”之前，先自己写修改清单，再比较后面的重构。

## 本章总结：把每个 P 压缩成一张决策卡

### P43 · 组合（一）：用同一接口表示叶子与容器

**问题**：遍历文件时到处判断当前节点是文件还是目录。

**动作**：让 Leaf 与 Composite 实现统一 Node 接口。

**验证**：调用方可以用递归统一计算大小或执行操作。

**代价**：叶子不支持的修改方法不应假装成功。

### P44 · 组合（二）：递归计算权限树与菜单树

**问题**：权限树的父节点和叶子节点使用完全不同的处理流程。

**动作**：在组合节点递归聚合孩子结果，叶子提供基本结果。

**验证**：任意子树都能作为整体处理。

**代价**：深树要注意循环引用、栈深和批量查询性能。

这些卡片故意把“代价”放在“验证”后面。先证明边界解决了真实问题，再讨论多了一层接口、注册表、包装器或组合根是否值得。没有代价说明的设计总结，通常只是模式宣传。

![组合模式与递归对象树：让叶子和容器接受同一种操作：同一案例随需求逐步演进](/images/notes/programming-thoughts/diagrams/chapter-composite-recursive-tree.svg)

## 起点：先写最直接、能工作的版本

这一版故意把决策和实现放在一起。它不是“错误代码”，而是需求尚少时合理的最低成本方案。请先运行它，并标出每个业务决定；接下来的 P43–P44 会连续修改同一案例。

```python
def allows(node, action):
...
## 第 1 轮需求：先解决问题，最后再叫它 Composite（P43）

我们继续修改同一个“同时包含用户、角色组和组织节点的权限树”，不另起一个玩具项目。本轮的新压力是：**调用方为用户、用户组和组织节点分别写类型分支**。先不要背模式名，先列出要改的判断、对象、外部依赖和测试；这份清单就是“修改半径”。

这一轮真正要回答的是：调用方如何不区分单个对象和对象集合就完成递归操作？ 这是本章的第一轮，先保留最直接的实现，记录它面对下一次需求时会触碰哪些地方。

### 先记录修改半径

- 当前泄漏：遍历文件时到处判断当前节点是文件还是目录。
- 设计动作：让 Leaf 与 Composite 实现统一 Node 接口。
- 稳定结果：调用方可以用递归统一计算大小或执行操作。
- 仍需承担的代价：叶子不支持的修改方法不应假装成功。

如果四项里只有“新增一个类”而没有说明哪种变化被隔离，先不要合并这次重构。设计的证据不是角色数量，而是下一次同类需求能否只新增实现、只替换适配器或只调整组合根。

### 这一轮只做一个设计动作

本轮只落实 **组合（一）：用同一接口表示叶子与容器**，围绕“Composite、树结构、Leaf、递归”移动一个边界，不同时引入后续模式。把稳定流程写成调用方真正需要的业务语言；把供应商字段、创建细节或横切动作留在边界另一侧。

完成动作后再给它命名：这一轮对应 **Composite、树结构、Leaf、递归**。名称只是压缩沟通，不是推导起点。如果无法从“需求如何变化”推回这个结构，就说明我们只记住了答案。
...
先写一个会失败的测试，再写实现：新增一种“文件与目录树”变化时，哪些稳定代码应该保持不动？如果实现抛出异常、返回空值或收到重复调用，调用方依赖的行为是否仍然成立？这一步能防止只验证 happy path。

本轮的验收标准是：调用方如何不区分单个对象和对象集合就完成递归操作？ 完成后，调用方可以用递归统一计算大小或执行操作。如果测试仍然必须启动所有外部服务，或仍要修改同一个大分支，说明这一步只换了名字，没有形成可验证的边界。

### 与前后两轮的连接

下一轮 P44 会继续检验这条边界：如果新增需求仍要修改本轮稳定代码，说明边界还没有放在真正的变化轴上。

不要把本轮结构当成永久答案。只要真实需求证明它没有减少修改范围，就应该回退一层，删除不必要的间接调用，再重新找变化轴。

## 第 2 轮需求：先解决问题，最后再叫它 递归遍历（P44）

我们继续修改同一个“同时包含用户、角色组和组织节点的权限树”，不另起一个玩具项目。本轮的新压力是：**权限树继续嵌套后，叶子和容器需要共享 allows() 递归协议**。先不要背模式名，先列出要改的判断、对象、外部依赖和测试；这份清单就是“修改半径”。

这一轮真正要回答的是：父节点如何聚合子节点结果，同时阻止叶子节点添加孩子？ 上一轮 P43 已经处理“Composite”；这一轮不要把它推倒重来，而是只处理新的变化轴。

### 先记录修改半径

- 当前泄漏：权限树的父节点和叶子节点使用完全不同的处理流程。
- 设计动作：在组合节点递归聚合孩子结果，叶子提供基本结果。
- 稳定结果：任意子树都能作为整体处理。
- 仍需承担的代价：深树要注意循环引用、栈深和批量查询性能。

如果四项里只有“新增一个类”而没有说明哪种变化被隔离，先不要合并这次重构。设计的证据不是角色数量，而是下一次同类需求能否只新增实现、只替换适配器或只调整组合根。

### 这一轮只做一个设计动作

本轮只落实 **组合（二）：递归计算权限树与菜单树**，围绕“递归遍历、透明性、安全性、聚合结果”移动一个边界，不同时引入后续模式。把稳定流程写成调用方真正需要的业务语言；把供应商字段、创建细节或横切动作留在边界另一侧。

完成动作后再给它命名：这一轮对应 **递归遍历、透明性、安全性、聚合结果**。名称只是压缩沟通，不是推导起点。如果无法从“需求如何变化”推回这个结构，就说明我们只记住了答案。
...
先写一个会失败的测试，再写实现：新增一种“组织权限树”变化时，哪些稳定代码应该保持不动？如果实现抛出异常、返回空值或收到重复调用，调用方依赖的行为是否仍然成立？这一步能防止只验证 happy path。

本轮的验收标准是：父节点如何聚合子节点结果，同时阻止叶子节点添加孩子？ 完成后，任意子树都能作为整体处理。如果测试仍然必须启动所有外部服务，或仍要修改同一个大分支，说明这一步只换了名字，没有形成可验证的边界。
...
![组合模式与递归对象树：让叶子和容器接受同一种操作：案例 UML 结构与对象关系](/images/notes/programming-thoughts/diagrams/chapter-composite-recursive-tree-uml.svg)
...
```python
tree = Group([User({"read"}), Group([User({"write"})])])
assert tree.allows("read")
assert tree.allows("write")
assert not tree.allows("delete")
```

![组合模式与递归对象树：让叶子和容器接受同一种操作：同一份代码重构前后结构对照](/images/notes/programming-thoughts/diagrams/chapter-composite-recursive-tree-comparison.svg)
...
为“同时包含用户、角色组和组织节点的权限树”增加一个与现有实现明显不同的需求。第一轮禁止增加接口，只修改朴素版本并记录触碰点；第二轮使用本文对应步骤重构；第三轮再加入一个变化，验证旧流程是否保持不动。

提交物包括：重构前后的修改清单、一张依赖方向图、至少两个业务测试、一个共享契约测试，以及一段不超过 150 字的代价说明。最后回看 P43–P44，确认每个结构都能追溯到一个真实失败，而不是追溯到一个想背下来的名词。

## 小结

本章真正形成的是一条推理链：直接实现 → 新需求暴露耦合 → 找到变化轴 → 放置最小边界 → 用测试证明 → 写下代价。做到这一步，Composite、递归遍历 才不再是彼此分散的知识点，而是同一段代码演进过程中自然出现的工具。
