...
| --- | --- | --- | --- |
| 1 | P43 | 调用方如何不区分单个对象和对象集合就完成递归操作？ | Composite |
| 2 | P44 | 父节点如何聚合子节点结果，同时阻止叶子节点添加孩子？ | 递归遍历 |
...
我们继续修改同一个“同时包含用户、角色组和组织节点的权限树”，不另起一个玩具项目。上一轮代码能工作，但新的需求让这个薄弱处出现：**调用方为用户、用户组和组织节点分别写类型分支**。先不要套类图，先写下如果沿用原结构，需要改哪些判断、对象和测试。

这一轮真正的问题是：调用方如何不区分单个对象和对象集合就完成递归操作？ 最直接的选择仍然是继续修改原函数；第一次这样做通常最便宜。等相同方向的修改再次出现，我们才获得足够证据，知道应该把哪部分从主流程中分离。

### 这一轮只做一个设计动作

本轮只落实 **组合（一）：用同一接口表示叶子与容器**：围绕“Composite、树结构、Leaf、递归”移动一个边界，不同时引入后续模式。验证标准也很具体——完成本轮需求后，再增加同方向实现时，上一轮已经稳定的业务结果不需要改写。
...
我们继续修改同一个“同时包含用户、角色组和组织节点的权限树”，不另起一个玩具项目。上一轮代码能工作，但新的需求让这个薄弱处出现：**权限树继续嵌套后，叶子和容器需要共享 allows() 递归协议**。先不要套类图，先写下如果沿用原结构，需要改哪些判断、对象和测试。

这一轮真正的问题是：父节点如何聚合子节点结果，同时阻止叶子节点添加孩子？ 最直接的选择仍然是继续修改原函数；第一次这样做通常最便宜。等相同方向的修改再次出现，我们才获得足够证据，知道应该把哪部分从主流程中分离。

### 这一轮只做一个设计动作

本轮只落实 **组合（二）：递归计算权限树与菜单树**：围绕“递归遍历、透明性、安全性、聚合结果”移动一个边界，不同时引入后续模式。验证标准也很具体——完成本轮需求后，再增加同方向实现时，上一轮已经稳定的业务结果不需要改写。
