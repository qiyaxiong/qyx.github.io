---
title: P39：代理（二）：在访问入口加入鉴权与缓存
description: 用“受保护的文档服务”案例理解保护代理，从坏味道、变化轴、重构结构到适用边界完成一次设计判断。
...
本节不从类图开始，而从 **受保护的文档服务** 的变化压力开始：每个调用点分别做鉴权、缓存和真实服务调用。真正要回答的问题是：哪些策略属于访问控制，哪些不应偷偷改变业务结果？

设计的目标不是消灭修改，而是让一次需求变化只触碰与它同方向的代码。下面这张图把本节决策压缩为“症状—动作—结果”三步。

![P39 代理（二）：在访问入口加入鉴权与缓存：从问题症状到设计动作与可验证结果](/images/notes/programming-thoughts/diagrams/p39-proxy-pattern-practice.svg)

## 核心判断

**保护代理** 在这里不是一条必须服从的口号。它是在提醒我们：保护代理集中访问策略，并委托给真实文档服务。其余关键词——缓存代理、权限、生命周期——都服务于同一个目的：找到一个调用方能理解、实现方能独立变化的边界。

重构之后期待的不是“类更多了”，而是一个可以验证的结果：安全与缓存策略有统一入口。如果做完重构仍然需要在五个文件里同步修改同一种规则，说明边界很可能放错了。
...
| 观察点 | 重构前 | 重构后 |
| --- | --- | --- |
| 变化位置 | 每个调用点分别做鉴权、缓存和真实服务调用 | 保护代理集中访问策略，并委托给真实文档服务 |
...
本节方案的主要代价是：授权结果应与用户和资源绑定，避免错误缓存。如果团队还说不清变化轴，先保留简单实现、收集第二个真实用例，通常比立即建立通用框架更安全。

另一个检查方法是看删除成本：如果抽象失败，能否在一次小重构中回到直接实现？越难撤销的设计，越需要真实证据。

## 动手练习

为“受保护的文档服务”再增加一个与现有实现差异明显的需求。先写出会被修改的文件清单，再用本节结构重构。要求至少写两个测试：一个验证稳定业务规则，一个验证新旧实现遵守相同契约。

最后用三句话复盘：变化轴是什么；边界为什么放在这里；为了这次解耦付出了什么复杂度。

## 本节小结

保护代理集中访问策略，并委托给真实文档服务。当你能用“变化原因”和“行为契约”解释这个决定时，保护代理 才从名词变成了可迁移的编程思想。
