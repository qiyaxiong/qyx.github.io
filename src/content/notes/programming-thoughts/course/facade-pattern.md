---
title: P40：外观模式：为复杂子系统提供稳定入口
description: 用“视频发布流水线”案例理解Facade，从坏味道、变化轴、重构结构到适用边界完成一次设计判断。
...
本节不从类图开始，而从 **视频发布流水线** 的变化压力开始：发布一段视频需要调用转码、封面、存储、通知四个子系统。真正要回答的问题是：如何减少调用方需要理解的对象数量，又不封死高级能力？

设计的目标不是消灭修改，而是让一次需求变化只触碰与它同方向的代码。下面这张图把本节决策压缩为“症状—动作—结果”三步。

![P40 外观模式：为复杂子系统提供稳定入口：从问题症状到设计动作与可验证结果](/images/notes/programming-thoughts/diagrams/p40-facade-pattern.svg)

## 核心判断

**Facade** 在这里不是一条必须服从的口号。它是在提醒我们：用 Facade 暴露面向用例的 publish() 入口。其余关键词——子系统、用例入口、简化协作——都服务于同一个目的：找到一个调用方能理解、实现方能独立变化的边界。

重构之后期待的不是“类更多了”，而是一个可以验证的结果：普通调用方只理解一个稳定流程。如果做完重构仍然需要在五个文件里同步修改同一种规则，说明边界很可能放错了。
...
| 观察点 | 重构前 | 重构后 |
| --- | --- | --- |
| 变化位置 | 发布一段视频需要调用转码、封面、存储、通知四个子系统 | 用 Facade 暴露面向用例的 publish() 入口 |
...
本节方案的主要代价是：外观不是万能上帝类，高级调用仍可使用底层接口。如果团队还说不清变化轴，先保留简单实现、收集第二个真实用例，通常比立即建立通用框架更安全。

另一个检查方法是看删除成本：如果抽象失败，能否在一次小重构中回到直接实现？越难撤销的设计，越需要真实证据。

## 动手练习

为“视频发布流水线”再增加一个与现有实现差异明显的需求。先写出会被修改的文件清单，再用本节结构重构。要求至少写两个测试：一个验证稳定业务规则，一个验证新旧实现遵守相同契约。

最后用三句话复盘：变化轴是什么；边界为什么放在这里；为了这次解耦付出了什么复杂度。

## 本节小结

用 Facade 暴露面向用例的 publish() 入口。当你能用“变化原因”和“行为契约”解释这个决定时，Facade 才从名词变成了可迁移的编程思想。
